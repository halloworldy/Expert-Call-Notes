...
DOCUMENT STRUCTURE:
The output must have two sections in this exact order:

1. BACKGROUND SECTION
- Start with the line "Background" (plain text, no numbering)
- Write a concise career background of the expert using information from:
  a. The EXPERT BIOGRAPHY data provided (if available)
  b. What the expert says about their own background in the transcript/notes
- Use a two-level bullet hierarchy only:
  * Top level: bullet character followed by main career point
  * Sub-level: "o" character followed by supporting detail
- Format example:
Background
* Started career 20 years ago in financial services technology
  o Joined Accenture in 2004 as a management consultant focused on payments
  o Progressed to senior manager before departing in 2010
* Currently serves as Chief Technology Officer at PayTech Solutions
  o Responsible for platform architecture and a team of 120 engineers
  o Has direct oversight of the issuing and acquiring infrastructure
- Keep the background section concise (typically 3-6 top-level bullets)
- Do NOT repeat background information in the Summary section

2. SUMMARY SECTION
- Start with the line "Summary" (plain text, no numbering)
...
Background
* [Career summary point]
  o [Supporting detail]
  o [Supporting detail]
* [Another career point]
  o [Detail]
...
- Use ONLY plain text formatting as described above
- The Background section uses * for top-level bullets and o for sub-bullets (indented with two spaces before the o)
- The Summary section uses numbered/lettered/roman format
- Section headers must be plain text after the number, e.g. "1. The company demonstrates strong recurring revenue characteristics"
- Sub-bullets must be plain text after the letter, e.g. "a. Revenue increased 45% year over year"
- Roman numeral items must be plain text after the numeral, e.g. "i. The largest customer accounts for approximately 8% of total revenue"
...
- Do not add any prefixes, labels, introductory text, or summary paragraphs outside the two sections
- Start directly with "Background" as the first line of output
